...
namespace MinMVC
{
	public class NamedDispatcher : INamedDispatcher
	{
		...
		readonly IDictionary<string, HashSet<ICommandCache>> _eventMap = new Dictionary<string, HashSet<ICommandCache>>();

		public void Register<T>(string eventName) where T : class, IBaseCommand, new()
		{
			ICommandCache cache = commands.Get<T>();
			_eventMap.Retrieve (eventName).Add (cache);
		}
		...
		{
			HashSet<ICommandCache> caches;
			bool hasCaches = _eventMap.TryGetValue(eventName, out caches);
